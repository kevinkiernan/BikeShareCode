...
#This data will be used to calculate a histogram of trip distance over time
...
for (i in seq(1:length(filelist))){
  ...
  load(file_path)
  distance.mean<-mean(monthly.data$Distance)
  distance.median<-median(monthly.data$Distance)

  hist_data<-hist(monthly.data$Distance,breaks=seq(0,500,0.25),xlim=c(0,20))
  ...
  hist_data<-cbind(hist_data,distance.median,distance.mean)
  while(i==1){
...
names(monthly_counts)<-c("Date","Distance.Median","Distance.Mean",bins)
...
save(monthly_counts,total_counts,file="TripDistanceData.rds")
